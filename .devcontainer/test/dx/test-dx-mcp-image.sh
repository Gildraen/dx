...
default_output="$(PATH="$workdir:/usr/bin:/bin" env -u GITHUB_MCP_IMAGE .devcontainer/src/dx/runtime/bin/dx-mcp github)"
[[ "$default_output" == *"ghcr.io/github/github-mcp-server:v1.10.1"* ]] || fail "default image is not pinned"

override_output="$(PATH="$workdir:/usr/bin:/bin" GITHUB_MCP_IMAGE="ghcr.io/github/github-mcp-server:test" .devcontainer/src/dx/runtime/bin/dx-mcp github)"
